...
Body_xxx_in_xxx_JSON_xxx_format:
	...
	The "existing-token" field is used to indicate that new token "SHOULD NOT" be generated;
...
CURL_xxx_examples:
	...
	1. Requesting token for data from a single resource server
		...
		3. server-token
			This is the token for the resource server, it can be ignored if
			request contains "id"s from a single resource-server.
		...
		Other_xxx_response_xxx_fields:
			...
			3. server-token
				...
				Important_xxx_note:
					...
					Hence, consumers must update their "server-token"s as in the API response.
