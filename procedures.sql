...
CREATE or REPLACE FUNCTION insert_groups(in provider_id text, in consumer_id text, in group_name text, in valid_till TIMESTAMP)
RETURNS void AS $$
BEGIN
   INSERT INTO groups
...
CREATE or REPLACE FUNCTION get_groups2(in provider_id text, in grp_name text, out consumer text, out valid_till numeric)
RETURNS record AS $$
BEGIN
...
CREATE or REPLACE FUNCTION get_groups_details(in provider_id text, out consumer text, out grp_name  text,out valid_till TIMESTAMP)
RETURNS record AS $$
BEGIN
